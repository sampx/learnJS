{
  ...
  console.log(str.split('').reverse().join(''));// 'y\uDE80\uD83Dx' 错误
...

{
  ...
  function Foo({id, name}){
    this.id = id;
    this.name = name;
    this.log = () =>{ //此处必须是箭头函数，否则就无法绑定this的值
      setTimeout(() =>{
  ...
  Foo.prototype.logme = function(){ //此处不能用箭头函数，因为箭头函数无自己的this
    setTimeout(() =>{ //此处必须是箭头函数，否则就无法绑定this的值
      console.log('id:', this.id, this.name);
    }, 100);
  }
...

{
  ...
  let sum_origin = function (x, y) { //递归函数
    if (y > 0) {
  ...
  let sum_tco = tco(function (x, y) { //递归函数
    if (y > 0) {
